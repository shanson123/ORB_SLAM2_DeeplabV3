...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &mask, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mImMask(mask),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    std::cout<<"---------------before delete , keypoints num : " << mvKeys.size() << std::endl;
	...
	//获取特征点的个数
    N = mvKeys.size();
    std::cout << "---------------after delete, keypoints num: " << N << std::endl;
